...
def computeTemperature(surfaceTemperature, depths, temperatures): 
    ...
    T = np.zeros(numnode)
    m_x = np.linspace(0, 1, len(temperatures))
    n_x = np.linspace(0, 1, len(T))
    T = np.interp(n_x, m_x, temperatures)
    T[0] = surfaceTemperature
    ...
    depthsTemperature = []
    for depth in depths:
        depthsTemperature.append(T[round(depth/dx)])

    print(depthsTemperature)

if __name__ == "__main__":
    input_data = json.loads(base64.b64decode(sys.argv[1]).decode("utf-8"))
    surfaceTemperature = input_data["surfaceTemperature"]
    depths = ast.literal_eval(input_data["depths"])
    temperatures = ast.literal_eval(input_data["temperatures"])

    computeTemperature(surfaceTemperature, depths, temperatures)
